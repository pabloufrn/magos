...
namespace canvas {
   ...
   struct Color
   {
       ...
       /// From hex string
       static Color from_hex_str(std::string str)
       {
            if(str.length() != 6)
                throw std::runtime_error("invalid color.");
            auto r = stoul(str.substr(0, 2), 0, 16);
            auto g = stoul(str.substr(2, 2), 0, 16);
            auto b = stoul(str.substr(4, 2), 0, 16);
            if(r > 255 or r > 255 or b > 255)
                throw std::runtime_error("invalid color.");
            return Color((Color::color_t) r, (Color::color_t) g, (Color::color_t) b);
       }
   };
